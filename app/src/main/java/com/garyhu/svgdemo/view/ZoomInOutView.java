...
public class ZoomInOutView extends View {
    /**
     * 初始化状态常量
     */
     ...
     * @param attrs
     */
    public ZoomInOutView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_POINTER_DOWN:
                Log.d("garyhu","pointerCount == "+event.getPointerCount());
            ...
            case MotionEvent.ACTION_MOVE:
                Log.d("garyhu","Move-->pointerCount == "+event.getPointerCount());
                if (event.getPointerCount() == 1) {
                    ...
                    if (totalTranslateX + movedDistanceX > 0) {
                        movedDistanceX = 0;
                    } else if (width - (totalTranslateX + movedDistanceX) > currentBitmapWidth-2) {
                        movedDistanceX = 0;
                    }
                    if (totalTranslateY + movedDistanceY > 0) {
                        movedDistanceY = 0;
                    } else if (height - (totalTranslateY + movedDistanceY) > currentBitmapHeight-2) {
                        movedDistanceY = 0;
                    }
                    // 调用onDraw()方法绘制图片
                    invalidate();
    ...
    private void zoom(Canvas canvas) {
        ...
        Log.d("garyhu","totalX == "+totalTranslateX);
        canvas.drawBitmap(sourceBitmap, matrix, null);
    }
